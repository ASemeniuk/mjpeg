...
public class MjpegView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public class MjpegViewThread extends Thread {
        ...
        private Rect destRect(int bmw, int bmh) {
            ...
            if (displayMode == MjpegView.SIZE_KEEP_ASPECT) {
                if (((float) bmw / (float) bmh < (float) dispWidth / (float) dispHeight)) { //Landscape
                    tempy = (bmh * dispWidth / bmw);
                    return new Rect(0, (dispHeight - tempy) / 2, dispWidth, (dispHeight + tempy) / 2);
                } else { //Portrait
                    tempx = (bmw * dispHeight / bmh);
                    return new Rect((dispWidth - tempx) / 2, 0, (dispWidth + tempx) / 2, dispHeight);
                }
            }
            return null;
        }
...
//                        }
                        ...
                        synchronized (mSurfaceHolder) {
                            c.clipRect(Math.max(0, destRect.left),
                                    Math.max(0, destRect.top),
                                    Math.min(dispWidth, destRect.right),
                                    Math.min(dispHeight, destRect.bottom));
